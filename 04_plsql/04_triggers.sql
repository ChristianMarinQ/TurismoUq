...
-- ---------------------------------------------------------------------
-- Disparador a NIVEL DE SENTENCIA: auditoría de cambios sobre TARIFA.
--
-- Un trigger de sentencia "puro" (sin FOR EACH ROW) no puede leer
-- :OLD/:NEW porque no se ejecuta por fila. Para cumplir "a nivel de
-- sentencia" y aun así registrar el valor anterior/nuevo de CADA fila
-- modificada, se usa un COMPOUND TRIGGER: la sección BEFORE EACH ROW solo
-- acumula los cambios en una colección PL/SQL (no escribe nada en la BD),
-- y la sección AFTER STATEMENT hace UN SOLO INSERT masivo (FORALL) al
-- terminar la sentencia completa.
--
-- Por qué importa que sea a nivel de sentencia y no fila por fila: si el
-- gerente actualiza el precio de 400 habitaciones de golpe
-- (UPDATE tarifa SET valor_noche = valor_noche * 1.1 WHERE ...), un
-- trigger de fila haría 400 INSERT individuales a auditoria_tarifa; este
-- hace 1 solo INSERT con las 400 filas, más eficiente y sin el riesgo de
-- "tabla en mutación" (ORA-04091) que aparecería si esa lógica intentara
...
-- ---------------------------------------------------------------------
-- Disparador a NIVEL DE FILA: impide que una habitación quede reservada
-- dos veces en fechas que se solapen. Es la última línea de defensa a
-- nivel de datos (sp_crear_reserva ya valida esto también, con un mensaje
-- más amigable vía RAISE_APPLICATION_ERROR) — este trigger protege la
--
-- Nota para la sustentación: este patrón (leer la misma tabla que el
-- trigger está modificando) funciona sin problema para INSERT/UPDATE de
-- una sola fila, como hace pkg_reservas.sp_crear_reserva (un INSERT por
-- habitación). Si alguien insertara varias filas de una vez con un
-- único INSERT...SELECT masivo, Oracle lanzaría ORA-04091 (tabla en
-- mutación), porque ahí sí habría ambigüedad sobre qué filas del propio
...
BEGIN
  ...
  IF v_conflictos > 0 THEN
    RAISE_APPLICATION_ERROR(-20002,
      'La habitación ' || :NEW.id_habitacion ||
