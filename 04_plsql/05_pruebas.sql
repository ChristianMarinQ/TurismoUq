...
-- Script de DEMOSTRACION para la sustentacion: no hay interfaz, asi que
...
-- fn_valor_estadia prorrateando por temporada (decision de diseno 2).
-- =====================================================================
DECLARE
  v_id_reserva NUMBER;
  v_id_hab1    NUMBER;
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('PRUEBA 2 FALLO: no debia dejar crear la reserva');
EXCEPTION
  WHEN OTHERS THEN
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('PRUEBA 3 FALLO: no debia dejar crear la reserva');
EXCEPTION
  WHEN OTHERS THEN
...
-- =====================================================================
-- PRUEBA 4: habitacion no disponible (solapamiento) -> ORA-20002
-- Se reserva una habitacion libre, y luego se intenta reservarla otra
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('Reserva A creada (' || v_id_reserva_a || ') sobre habitacion ' || v_id_hab);
  ...
  BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('PRUEBA 4 FALLO: no debia dejar crear la reserva B');
  EXCEPTION
    WHEN OTHERS THEN
...
-- =====================================================================
-- PRUEBA 5: el trigger trg_no_solape_reserva tambien protege aunque se
...
BEGIN
  ...
  -- Reserva C: encabezado + linea, todo "a mano" (sin pkg_reservas)
  ...
  DBMS_OUTPUT.PUT_LINE('Reserva C creada a mano (' || v_id_reserva_c || ') sobre habitacion ' || v_id_hab);
  ...
  BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('PRUEBA 5 FALLO: el trigger no bloqueo el INSERT directo');
  EXCEPTION
    WHEN OTHERS THEN
      IF SQLCODE = -20002 THEN
        DBMS_OUTPUT.PUT_LINE('PRUEBA 5 OK: trg_no_solape_reserva bloqueo el INSERT directo -> ' || SQLERRM);
        DELETE FROM reserva WHERE id_reserva = v_id_reserva_d; -- limpiar encabezado huerfano de la prueba
      ELSE
        RAISE;
      END IF;
  END;
...
-- (FORALL) con una fila de auditoria por cada tarifa modificada.
-- =====================================================================
DECLARE
...
-- =====================================================================
-- PRUEBA 7: sp_liquidacion_mensual - proceso masivo con cursor explicito
-- =====================================================================
